"""
Simple test of workflow execution with the new llama-3.3-70b-versatile model
"""
import requests
import json
import time
...
if response.status_code == 201:
    run = response.json()
    run_id = run['id']
    print(f"[OK] Created run ID: {run_id}")
...
if response.status_code == 200:
    print("[OK] Workflow execution started")
else:
...
for i in range(max_attempts):
    ...
    print("\n[TIMEOUT] Workflow is still running after 60 seconds")
